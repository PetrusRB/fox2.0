 ...
 * \version 1.0
 *
 *  RedisX is a completely free Redis / Valkey client library, available on
 * Github as:
 *
...
/// (bytes) Default TCP buffer size (send/recv) for Redis clients. Values &lt;=
/// 0 will use system default.
#define REDISX_TCP_BUF_SIZE 0
#endif

#ifndef REDISX_CMDBUF_SIZE
/// (bytes) Size of many internal arrays, and the max. send chunk size. At least
/// ~16 bytes...
#define REDISX_CMDBUF_SIZE 8192
#endif

#ifndef REDISX_RCVBUF_SIZE
/// (bytes) Redis receive buffer size (at most that much is read from the socket
/// in a single call).
...
/// You may want to set it quite high to ensure that the receive buffer is
/// promptly cleared.
...
#define REDISX_VERSION_STRING                                                  \
  str_2(REDISX_MAJOR_VERSION) "." str_2(REDISX_MINOR_VERSION) "." str_2(       \
      REDISX_PATCHLEVEL) REDISX_RELEASE_STRING

/**
 * Enumeration of RESP component types. These are the first character IDs for
 * the standard RESP interface implemented by Redis.
 *
 */
enum resp_type {
  // RESP2 types:
  ...
  RESP3_VERBATIM_STRING =
      '=', ///< \hideinitializer RESP3 verbatim string (with type)
  ...
  RESP3_ATTRIBUTE =
      '|', ///< \hideinitializer RESP3 dictionary of attributes (metadata)
  RESP3_PUSH =
      '>', ///< \hideinitializer RESP3 dictionary of attributes (metadata)
};
...
#define REDIS_INVALID_CHANNEL                                                  \
  (-101) ///< \hideinitializer There is no such channel in the Redis instance.
...
#define REDIS_INCOMPLETE_TRANSFER                                              \
  (-104) ///< \hideinitializer The transfer to/from Redis is incomplete
#define REDIS_UNEXPECTED_RESP                                                  \
  (-105) ///< \hideinitializer Got a Redis response of a different type than
         ///< expected
#define REDIS_UNEXPECTED_ARRAY_SIZE                                            \
  (-106) ///< \hideinitializer Got a Redis response with different number of
         ///< elements than expected.
#define REDIS_MOVED                                                            \
  (-107) ///< \hideinitializer The requested key has moved to another cluster
         ///< shard.
#define REDIS_MIGRATING                                                        \
  (-108) ///< \hideinitializer The requested key is importing, and you may query
         ///< with ASKED on the specified node.

/**
 * RedisX channel IDs. RedisX uses up to three separate connections to the
 * server: (1) an interactive client, in which each query is a full round trip,
 * (2) a pipeline client, in which queries are submitted in bulk, and responses
 * arrive asynchronously, and (3) a subscription client devoted to PUB/SUB
 * requests and push messages. Not all clients are typically initialized at
 * start. The interactive channel is always connected; the pipeline client can
 * be selected when connecting to the server; and the subscription client is
 * connected as needed to process PUB/SUB requests.
 */
enum redisx_channel {
  REDISX_INTERACTIVE_CHANNEL =
      0, ///< \hideinitializer Redis channel number for interactive queries
  REDISX_PIPELINE_CHANNEL,    ///< \hideinitializer Redis channel number for
                              ///< pipelined transfers
  REDISX_SUBSCRIPTION_CHANNEL ///< \hideinitializer Redis channel number for
                              ///< PUB/SUB messages
};

#define REDISX_CHANNELS                                                        \
  (REDISX_SUBSCRIPTION_CHANNEL +                                               \
   1) ///< \hideinitializer The number of channels a Redis instance has.

/**
 * The RESP protocol to use for a Redis instance. Redis originally used RESP2,
 * but later releases added support for RESP3.
 *
 */
...
typedef struct RESP {
  ...
  int n; ///< Either the integer value of a RESP_INT or a RESP3_BOOLEAN
         ///< response, or the dimension of the value field.
  void *value; ///< Pointer to text (char *) content or to an array of
               ///< components (RESP**) or (RedisMap *), or else a pointer to a
...
typedef struct RedisClient {
  /// \cond PRIVATE
  void *priv; ///< Private data for this Redis client not exposed by the public
              ///< API
  /// \endcond
} RedisClient;

/**
 * \brief Structure that represents a Redis database instance, with one or more
 * RedisClient connections.
 *
 * \sa redisxInit()
 * \sa redisxDestroy()
 */
typedef struct Redis {
  char *id; ///< The string ID of the Redis server. Default is IP, e.g.
            ///< "127.0.0.1"
  ...
  /// \cond PROTECTED
  void *priv; ///< Private data for this Redis instance not exposed by the
              ///< public API
  /// \endcond
} Redis;

/**
 * A type of function that handles Redis PUB/SUB messages. These functions
 * should follow a set of basic rules:
 *
 * <ul>
 * <li>The call should return promptly and never block for significant periods.
 * If it has blocking calls or if extended processing is required, the function
 * should simply place a copy of the necessary information on a queue and
 * process queued entries in a separate thread. (The call arguments will not
 * persist beyond the scope of the call, so don't attempt to place them directly
 * in a queue.)</li>
 *
 * <li>The subscriber call should not attempt to modify or free() the strings it
 * is called with. The same strings maybe used by other subscribers, and thus
 * modifying their content would produce unpredictable results with those
 * subscribers.</li>
 *
 * <li>If the call needs to manipulate the supplied string arguments, it should
 * operate on copies (e.g. obtained via xStringCopy()).</li>
 *
 * <li>The caller should free up any temporary resources it allocates, including
 * copies of the argument strings, before returning. However, it should never
 * call free() on the supplied arguments directly.</li>
 * </ul>
 *
 ...
 *                 be binary a '\0' termination should not be assumed. Instead,
 * the length of the message is specified explicitly.
 */
typedef void (*RedisSubscriberCall)(const char *pattern, const char *channel,
                                    const char *msg, long length);

/**
 * User-specified callback function for handling RedisX errors from socket-level
 * read / write calls. It's mainly there for the application to perform any
 * cleanup as necessary or to report the error. However, it can also interact
 * with the Redis instance in limited ways. The implementation should follow a
 * set of basic rules:
 *
 * <ul>
 * <li>It should not call synchronized functions on the affected client,
 * including attempts to connect, disconnect, or reconnect the client.</li>
 * <li>It may call `Async` functions for the client (the client is in a locked
 * state when the handler is called).</li> <li>It should not attempt to lock or
 * unlock the affected client.</li> <li>It may use `errno` to gather information
 * about the source of the error, and may even change or reset `errno`, to
 * change behavior (e.g. re-setting to `EAGAIN` or `EWOULBLOCK` will keep the
 * client connected and the return status of the failed call will become
 * X_TIMEDOUT; or setting it to any other value will disconnect the client and
 * the failed call will return X_NO_SERVICE instead.).</li> <li>The limitations
 * can by bypassed by placing the error on a queue and let an asynchronous
 * thread take it from there.</li>
 * </ul>
 *
 ...
 */
typedef void (*RedisErrorHandler)(Redis *redis, enum redisx_channel channel,
                                  const char *op);

/**
 * A user-defined function for consuming responses from a Redis pipeline
 * connection. The implementation should follow a set of simple rules:
 *
 * <ul>
 * <li>the implementation should not destroy the RESP data. The RESP will be
 * destroyed automatically after the call returns. However, the call may retain
 * any data from the RESP itself, provided the data is de-referenced from the
 * RESP before return.<li> <li>The implementation should not block (aside from
 * maybe a quick mutex unlock) and return quickly, so as to not block the client
 * for long periods</li> <li>If extensive processing or blocking calls are
 * required to process the message, it is best to simply place a copy of the
 * RESP on a queue and then return quickly, and then process the message
...
/**
 * A user-defined function for consuming push messages from a Redis client. The
 * implementation should follow a set of simple rules:
 *
 * <ul>
 * <li>the implementation should not destroy the RESP data. The RESP will be
 * destroyed automatically after the call returns. However, the call may retain
 * any data from the RESP itself, provided that such data is de-referenced from
 * the RESP before the return.<li> <li>The implementation should not block
 * (aside from maybe a quick mutex unlock) and return quickly, so as to not
 * block the client for long periods</li> <li>If extensive processing or
 * blocking calls are required to process the message, it is best to simply
 * place a copy of the RESP on a queue and then return quickly, and then process
 * the message asynchronously in a background thread.</li> <li>The client on
 * which the push notification originated will be locked when this function is
 * called, waiting for a response to an earlier query. Thus the implementation
 * should not attempt to lock the client again or release the lock. It may send
 * asynchronous requests on the client, e.g. via redisxSendRequestAsync(), but
 * it should not try to read a response (given that the client is blocked for
 * another read operation). If more flexible client access is needed, the
 * implementation should make a copy of the RESP and place it on a queue for
 * asynchronous processing by another thread.
 * </li>
 * </ul>
 *
 * @param cl          The Redis client that sent the push. The client is locked
 * for exlusive access when this function is called.
...
/**
 * User callback function allowing additional customization of the client socket
 * before connection.
 *
 * @param socket      The socket descriptor.
 * @param channel     REDISX_INTERACTIVE_CHANNEL, REDISX_PIPELINE_CHANNEL,
 * REDISX_SUBSCRIPTION_CHANNEL
 * @return            X_SUCCESS (0) if the socket may be used as is after the
 * return. Any other value will indicate that the socket should not be used and
 * that the caller itself should fail with an error.
 */
...
Redis *redisxInitSentinel(const char *serviceName,
                          const RedisServer *serverList, int nServers);
...
int redisxSetMutualTLS(Redis *redis, const char *cert_file,
                       const char *key_file);
...
Redis *redisxClusterGetRedirection(RedisCluster *cluster, const RESP *redirect,
                                   boolean refresh);
RESP *redisxClusterAskMigrating(Redis *redis, const char **args,
                                const int *lengths, int n, int *status);
...
RESP *redisxRequest(Redis *redis, const char *command, const char *arg1,
                    const char *arg2, const char *arg3, int *status);
RESP *redisxArrayRequest(Redis *redis, const char **args, const int *length,
                         int n, int *status);
RESP *redisxGetAttributes(Redis *redis);
int redisxGetAvailable(RedisClient *cl);
int redisxSetValue(Redis *redis, const char *table, const char *key,
                   const char *value, boolean confirm);
RESP *redisxGetValue(Redis *redis, const char *table, const char *key,
                     int *status);
char *redisxGetStringValue(Redis *redis, const char *table, const char *key,
                           int *len);
RedisEntry *redisxGetTable(Redis *redis, const char *table, int *n);
RedisEntry *redisxScanTable(Redis *redis, const char *table,
                            const char *pattern, int *n);
int redisxMultiSet(Redis *redis, const char *table, const RedisEntry *entries,
                   int n, boolean confirm);
...
int redisxPublish(Redis *redis, const char *channel, const char *message,
                  int length);
...
int redisxAddSubscriber(Redis *redis, const char *channelStem,
...
RESP *redisxRunScript(Redis *redis, const char *sha1, const char **keys,
                      const char **params, int *status);
...
int redisxCheckRESP(const RESP *resp, enum resp_type expectedType,
                    int expectedSize);
...
void redisxPrintDelimited(const RESP *resp, const char *delim,
...
// Asynchronous access routines (use within redisxLockClient()/
// redisxUnlockClient() blocks)...
int redisxSendRequestAsync(RedisClient *cl, const char *command,
                           const char *arg1, const char *arg2,
                           const char *arg3);
int redisxSendArrayRequestAsync(RedisClient *cl, const char **args,
                                const int *length, int n);
int redisxClusterAskMigratingAsync(RedisClient *cl, const char **args,
                                   const int *lengths, int n);
int redisxSetValueAsync(RedisClient *cl, const char *table, const char *key,
                        const char *value, boolean confirm);
int redisxMultiSetAsync(RedisClient *cl, const char *table,
                        const RedisEntry *entries, int n, boolean confirm);
...
int redisxPublishAsync(Redis *redis, const char *channel, const char *data,
                       int length);
